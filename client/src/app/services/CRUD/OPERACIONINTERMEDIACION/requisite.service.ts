...
export class RequisiteService {
   ...
   url = environment.api_operacionintermediacion + 'requisite/';
   ...
   post(requisite: Requisite): Promise<any> {
      return this.http.post(this.url, JSON.stringify(requisite), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(requisite: Requisite): Promise<any> {
      return this.http.put(this.url, JSON.stringify(requisite), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
