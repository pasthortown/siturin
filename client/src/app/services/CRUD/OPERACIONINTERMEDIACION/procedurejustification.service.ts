...
import { ProcedureJustification } from './../../../models/OPERACIONINTERMEDIACION/ProcedureJustification';

@Injectable({
   providedIn: 'root'
})
export class ProcedureJustificationService {

   url = environment.api_operacionintermediacion + 'procedurejustification/';
   ...
   post(procedurejustification: ProcedureJustification): Promise<any> {
      return this.http.post(this.url, JSON.stringify(procedurejustification), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(procedurejustification: ProcedureJustification): Promise<any> {
      return this.http.put(this.url, JSON.stringify(procedurejustification), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
