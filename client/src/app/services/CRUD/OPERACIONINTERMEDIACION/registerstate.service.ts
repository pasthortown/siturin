...
import { RegisterState } from './../../../models/OPERACIONINTERMEDIACION/RegisterState';
...
export class RegisterStateService {
   ...
   url = environment.api_operacionintermediacion + 'registerstate/';
   ...
   post(registerstate: RegisterState): Promise<any> {
      return this.http.post(this.url, JSON.stringify(registerstate), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(registerstate: RegisterState): Promise<any> {
      return this.http.put(this.url, JSON.stringify(registerstate), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
