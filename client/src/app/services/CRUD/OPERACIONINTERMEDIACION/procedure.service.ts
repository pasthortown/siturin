...
import { Procedure } from './../../../models/OPERACIONINTERMEDIACION/Procedure';
...
export class ProcedureService {
   ...
   url = environment.api_operacionintermediacion + 'procedure/';
   ...
   post(procedure: Procedure): Promise<any> {
      return this.http.post(this.url, JSON.stringify(procedure), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(procedure: Procedure): Promise<any> {
      return this.http.put(this.url, JSON.stringify(procedure), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
