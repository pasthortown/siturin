...
export class TuristicTransportService {

   url = environment.api_operacionintermediacion + 'turistictransport/';
   ...
   post(turistictransport: TuristicTransport): Promise<any> {
      return this.http.post(this.url, JSON.stringify(turistictransport), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(turistictransport: TuristicTransport): Promise<any> {
      return this.http.put(this.url, JSON.stringify(turistictransport), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
