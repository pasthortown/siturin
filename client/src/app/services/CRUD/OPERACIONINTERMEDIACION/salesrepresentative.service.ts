...
export class SalesRepresentativeService {
   ...
   url = environment.api_operacionintermediacion + 'salesrepresentative/';
   ...
   post(salesrepresentative: SalesRepresentative): Promise<any> {
      return this.http.post(this.url, JSON.stringify(salesrepresentative), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(salesrepresentative: SalesRepresentative): Promise<any> {
      return this.http.put(this.url, JSON.stringify(salesrepresentative), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
