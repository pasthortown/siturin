...
import { RegisterTypeImage } from './../../../models/OPERACIONINTERMEDIACION/RegisterTypeImage';

@Injectable({
   providedIn: 'root'
})
export class RegisterTypeImageService {

   url = environment.api_operacionintermediacion + 'registertypeimage/';
   ...
   post(registertypeimage: RegisterTypeImage): Promise<any> {
      return this.http.post(this.url, JSON.stringify(registertypeimage), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(registertypeimage: RegisterTypeImage): Promise<any> {
      return this.http.put(this.url, JSON.stringify(registertypeimage), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
