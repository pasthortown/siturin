...
export class FoodDrinkAttachmentService {

   url = environment.api_alimentosbebidas + 'fooddrinkattachment/';
   ...
   post(fooddrinkattachment: FoodDrinkAttachment): Promise<any> {
      return this.http.post(this.url, JSON.stringify(fooddrinkattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(fooddrinkattachment: FoodDrinkAttachment): Promise<any> {
      return this.http.put(this.url, JSON.stringify(fooddrinkattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
