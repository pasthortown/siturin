...
export class ServiceTypeService {
   ...
   url = environment.api_alimentosbebidas + 'servicetype/';
   ...
   post(servicetype: ServiceType): Promise<any> {
      return this.http.post(this.url, JSON.stringify(servicetype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(servicetype: ServiceType): Promise<any> {
      return this.http.put(this.url, JSON.stringify(servicetype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
