...
export class KitchenTypeService {
   ...
   url = environment.api_alimentosbebidas + 'kitchentype/';
   ...
   post(kitchentype: KitchenType): Promise<any> {
      return this.http.post(this.url, JSON.stringify(kitchentype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(kitchentype: KitchenType): Promise<any> {
      return this.http.put(this.url, JSON.stringify(kitchentype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
