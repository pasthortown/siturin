...
export class RegisterTypeService {
   ...
   url = environment.api_alimentosbebidas + 'registertype/';
   ...
   get_by_register_type(register_type_id: number): Promise<any> {
      return this.http.get(this.url + 'get_by_register_type?register_type_id=' + register_type_id.toString(), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json());  });
   }
   ...
   post(registertype: RegisterType): Promise<any> {
      return this.http.post(this.url, JSON.stringify(registertype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(registertype: RegisterType): Promise<any> {
      return this.http.put(this.url, JSON.stringify(registertype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
