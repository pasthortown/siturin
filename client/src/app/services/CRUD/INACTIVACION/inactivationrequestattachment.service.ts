...
export class InactivationRequestAttachmentService {

   url = environment.api_inactivacion + 'inactivationrequestattachment/';
   ...
   post(inactivationrequestattachment: InactivationRequestAttachment): Promise<any> {
      return this.http.post(this.url, JSON.stringify(inactivationrequestattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(inactivationrequestattachment: InactivationRequestAttachment): Promise<any> {
      return this.http.put(this.url, JSON.stringify(inactivationrequestattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
