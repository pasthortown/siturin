...
export class InactivationRequestService {

   url = environment.api_inactivacion + 'inactivationrequest/';
   ...
   post(inactivationrequest: InactivationRequest): Promise<any> {
      return this.http.post(this.url, JSON.stringify(inactivationrequest), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(inactivationrequest: InactivationRequest): Promise<any> {
      return this.http.put(this.url, JSON.stringify(inactivationrequest), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
