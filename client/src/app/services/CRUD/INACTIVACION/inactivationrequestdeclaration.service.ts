...
export class InactivationRequestDeclarationService {

   url = environment.api_inactivacion + 'inactivationrequestdeclaration/';
   ...
   post(inactivationrequestdeclaration: InactivationRequestDeclaration): Promise<any> {
      return this.http.post(this.url, JSON.stringify(inactivationrequestdeclaration), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(inactivationrequestdeclaration: InactivationRequestDeclaration): Promise<any> {
      return this.http.put(this.url, JSON.stringify(inactivationrequestdeclaration), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
