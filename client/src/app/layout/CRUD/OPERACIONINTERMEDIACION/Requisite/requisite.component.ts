...
import { Requisite } from './../../../../models/OPERACIONINTERMEDIACION/Requisite';
...
export class RequisiteComponent implements OnInit {
   ...
   toCSV() {
      this.requisiteDataService.get().then( r => {
         ...
         let output = 'id;name;description;father_code;to_approve;mandatory;type;params;code;score;register_type_id;is_adventure_requisite\n';
         backupData.forEach(element => {
            output += element.id; + element.name + ';' + element.description + ';' + element.father_code + ';' + element.to_approve + ';' + element.mandatory + ';' + element.type + ';' + element.params + ';' + element.code + ';' + element.score + ';' + element.register_type_id + ';' + element.is_adventure_requisite + '\n';
         });
